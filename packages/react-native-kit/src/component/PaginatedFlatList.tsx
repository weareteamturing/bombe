...
export type PaginatedFlatListRef = {
  scrollToTop: (arg?: { animated: boolean }) => void;
  scrollToOffset: (param: { offset: number; animated?: boolean }) => void;
 ...
 */
export const PaginatedFlatList = React.forwardRef<PaginatedFlatListRef, PaginatedFlatListProps<any>>((props, ref) => {
  const {
    data,
    ListHeaderComponent,
    ListFooterComponent,
  ...
  useImperativeHandle(
    ...
    () => ({
      scrollToTop: (arg?: { animated: boolean }) => {
        flatListRef.current?.scrollToOffset({ animated: arg?.animated || false, offset: 0 });
      },
      scrollToOffset: ({ offset, animated = false }: { offset: number; animated?: boolean }) => {
