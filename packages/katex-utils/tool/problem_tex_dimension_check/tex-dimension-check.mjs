  ...
  if (element) {
    ...
    if (screenshot) {
      await page.screenshot({
        path: `tool/data/screenshot/${id}.png`,
        type: 'png',
        fullPage: false,
...
async function go() {
  for (const p of problems.slice(offset)) {
    i++;
    const { problem_tex, task_id, id, solution_tex } = p;
    const widthProblem = await checkWidth(`${id}-${task_id}-p`, problem_tex);
    const widthSolution = await checkWidth(`${id}-${task_id}-s`, solution_tex);
    ...
    if ((i % 1000 === 0 && i) || i + offset === problems.length - 1) {
      result.sort((a, b) => {
        return Math.max(b.widthProblem, b.widthSolution) - Math.max(a.widthProblem, a.widthSolution);
      });
      console.log(result.length);
      const maximumProblemId = result[0].problem.id;
      const { problem_tex, task_id, id, solution_tex } = problems.find((p) => p.id === maximumProblemId);
      await checkWidth(`${id}-${task_id}-p`, problem_tex, true);
      await checkWidth(`${id}-${task_id}-s`, solution_tex, true);

      writeJson(`tool/problem_tex_dimension_check/measure-result-${i + offset}.json`, result);
      result = [];
      print(i + offset);
    }
  }
...
// const ret = [];
// for (let i = 1000; i < 79000; i += 1000) {
//   const tmp = readJsonSlow(`tool/data/measure-result-${i}.json`);
//   print(tmp.length);
//   ret.push(...tmp);
// }
// for (let i = 79600; i < 80000; i += 80000) {
//   const tmp = readJsonSlow(`tool/data/measure-result-${i}.json`);
//   print(tmp.length);
//   ret.push(...tmp);
// }
...
//   return Math.max(b.widthProblem, b.widthSolution) - Math.max(a.widthProblem, a.widthSolution);
// });
// writeJson('tool/data/measure-result.json', ret);
